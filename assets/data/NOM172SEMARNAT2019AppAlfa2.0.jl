...
function writeFile(text)
  logEvent("PROCESS", "Writing JSON from WebScrapping")
	io = open(joinpath(@__DIR__, "output.json"), "w");
	write(io, text);
	close(io)
...
function getCriteriaPollutants(stationID)
  ...
  for pollutant in criteriaPollutants
    ...
    dataLine = ""
    try
      response = HTTP.request("POST", url, ["Content-Type" => "application/x-www-form-urlencoded", "charset" => "UTF-8"], HTTP.URIs.escapeuri(params), require_ssl_verification =false)
...
function retrieveData()
  ...
  try
    ...
    response = HTTP.request("GET", generalDataURL, require_ssl_verification =false)
...
function readDataFile()
	io = open(joinpath(@__DIR__, "output.json"), "r");
	text = read(io, String);
...
function writeAnalysisFile(data)
	io = open( joinpath(@__DIR__,"analysis.json"), "w");
	text = JSON.json(data)
	write(io, text);
	close(io)
end
       ...
       logEvent("ANALYZING DATA",state["NOMBRE"]*"-"*estacion["NOMBRE"])
        ...
        if length( keys( contaminantes["PM10"] )) > 0       
          for valinfo in contaminantes["PM10"]
            append!(concentrationPM10,valinfo["valor"])
            append!(hourPM10,valinfo["hora"])
          ...
          if length(promMovil12PM10T) > 0
             ...
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"promMovil12PM10T.txt") , hourPM10, promMovil12PM10T)
          end  
          if length(concentrationPM10) > 0
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"concentrationPM10.txt" ) , hourPM10, concentrationPM10)
          end         
        ...
        if length( keys( contaminantes["PM2.5"] )) > 0
          for valinfo in contaminantes["PM2.5"]
            println(valinfo["valor"])
            #append!(concentrationPM25,parse(Float64, valinfo["valor"]))
            append!(concentrationPM25,valinfo["valor"])
            # append!(hourPM25,parse(Float64,valinfo["hora"]))
            append!(hourPM25,valinfo["hora"])
          ...
          if length(promMovil12PM25T) > 0
             ...
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"promMovil12PM25T.txt"), hourPM25, promMovil12PM25T)
          end
          if length(concentrationPM25) > 0
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"concentrationPM25.txt") , hourPM25, concentrationPM25)
          end         
        ...
        if length( keys( contaminantes["CO"] )) > 0
          for valinfo in contaminantes["CO"]
            append!(concentrationCO,valinfo["valor"])
            append!(hourCO,valinfo["hora"])
          ...
          if length(promMovil8COT) > 0
             ...
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"promMovil8COT.txt") , hourCO, promMovil8COT)
          end
          if length(concentrationCO) > 0
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"concentrationCO.txt") , hourCO, concentrationCO)
          end         
        ...
        if length( keys( contaminantes["NO2"] )) > 0
          for valinfo in contaminantes["NO2"]
            append!(concentrationNO2,valinfo["valor"])
            append!(hourNO2,valinfo["hora"])
          ...
          if length(concentrationNO2) > 0
             ...
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"promHorariaNO2.txt") , hourNO2, concentrationNO2)
          end
        ...
        if length( keys( contaminantes["O3"] )) > 0
          for valinfo in contaminantes["O3"]
            append!(concentrationO3,valinfo["valor"])
            append!(hourO3,valinfo["hora"])
          ...
          if length(promMovil8O3T) > 0
             ...
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"promMovil8O3T.txt"), hourO3, promMovil8O3T)
          end
          if length(concentrationO3) > 0
             histCAO3H = histCA(concentrationO3, O3LH)          
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"concentrationO3.txt"), hourO3, concentrationO3)
          end         
        ...
        if length( keys( contaminantes["SO2"] )) > 0
          for valinfo in contaminantes["SO2"]
            append!(concentrationSO2,valinfo["valor"])
            append!(hourSO2,valinfo["hora"])
          ...
          if length(promMovil24SO2T) > 0
             ...
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"promMovil24SO2T.txt"), hourSO2, promMovil24SO2T)
          end
          if length(concentrationSO2) > 0
             writeTS( joinpath(@__DIR__,"prom/"*red["CODIGO"]*"concentrationSO2.txt"), hourSO2, concentrationSO2)
          end          
...
function dumpSummary()
  io = open(joinpath(@__DIR__, "analysis.json"), "r");
  text = read(io, String);
  ...
  for station in stations
    ...
    file = open( joinpath(@__DIR__,"dump/dump"*station["IDSTATION"]*".txt" ), "w")
    println(file,"***************************************************************")
